...
#define log_err(fmt, ...)                                                      \
    fprintf(stderr, "[sharc-booter] error: " fmt "\n", ##__VA_ARGS__)
...
static struct gpiod_line_request *request_output_line(
...
static bool
transfer(int spi_device_fd, uint8_t *tx, uint8_t *rx, unsigned long bufSize);
static unsigned char reverse_bit_order(unsigned char b);
...
int main(int argc, char **argv) {
    ...
    if (*p != '\0' || errno) {
        log_err("Invalid sharc index %s", argv[2]);
        return 1;
    }

    if ((sharc_index < 0) || (sharc_index >= N_SHARCS)) {
        log_err("Invalid sharc index %lu", sharc_index);
        return 1;
    }

    FILE *firmware = fopen(argv[1], "rb");
    if (!firmware) {
        log_err("Failed to open firmware file %s", argv[1]);
        return 1;
    }
    ...
    if (!reset_sharc(sharc_index)) {
        log_err("Failed to reset sharc");
        return 1;
    }

    if (!load_file(spi_device_fd, firmware, INITIAL_FIRMWARE_LOAD_SIZE)) {
        log_err("Failed to write initial firmware blob");
        return 1;
    }
    ...
    if (!load_file(spi_device_fd, firmware, 1000000000)) {
        log_err("Failed to write rest of firmware blob");
        return 1;
    }
...
static struct gpiod_line_request *request_output_line(
    ...
    if (!chip) {
        log_err("failed to open chip");
        return NULL;
    }

    struct gpiod_line_settings *settings = gpiod_line_settings_new();
    if (!settings) {
        log_err("failed to create line settings");
        goto close_chip;
    }
    ...
    if (!line_cfg) {
        log_err("failed to create line config");
        goto free_settings;
    }
    int ret =
        gpiod_line_config_add_line_settings(line_cfg, &offset, 1, settings);
    if (ret) {
        log_err("failed to add settings to line config");
        goto free_line_config;
    }
    struct gpiod_request_config *req_cfg = gpiod_request_config_new();
    if (!req_cfg) {
        log_err("failed to create request config");
        goto free_line_config;
    }
...
static bool reset_sharc(int sharc_index) {
    ...
    struct gpiod_line_request *request = request_output_line(
        SHARC_RESET_CHIP, reset_line_offset, GPIOD_LINE_VALUE_ACTIVE,
        "sharc-reset"
    );
    if (!request) {
        log_err("reset_sharc: failed to request line: %s", strerror(errno));
        return false;
    }

    gpiod_line_request_set_value(
        request, reset_line_offset, GPIOD_LINE_VALUE_INACTIVE
    );
    usleep(1000);
    gpiod_line_request_set_value(
        request, reset_line_offset, GPIOD_LINE_VALUE_ACTIVE
    );
    usleep(1000);
...
static int init_spi(char const *device_filename) {
    ...
    if (fd < 0) {
        log_err("couldn't open device file");
        return fd;
    }
    ...
    if (ret < 0) {
        log_err("can't set spi bits per word");
        return ret;
    }

    ret = ioctl(fd, SPI_IOC_RD_BITS_PER_WORD, &bits);
    if (ret < 0) {
        log_err("can't get spi bits per word");
        return ret;
    }
    ...
    if (ret < 0) {
        log_err("can't set spi max speed");
        return ret;
    }

    ret = ioctl(fd, SPI_IOC_RD_MAX_SPEED_HZ, &speed);
    if (ret < 0) {
        log_err("can't get spi max speed");
        return ret;
    }
    return fd;
}
...
static bool load_file(int spi_device_fd, FILE *fp, int size) {
    ...
    while (size > 0 &&
           (readed = fread(buf, 4, min(BUF_SIZE_WORDS, size), fp)) > 0) {
        for (int i = 0; i < readed * 4; i++)
            tx[i] = reverse_bit_order(buf[i]);
...
transfer(int spi_device_fd, uint8_t *tx, uint8_t *rx, unsigned long bufSize) {
    ...
    if (ret < 1) {
        log_err("failed to execute spi transfer\n");
        return false;
    }
    return true;
}
